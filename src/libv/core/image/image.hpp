...
template<class T>
class Image
  : public image_::image_layout<Image<T>, typename T::scalar_type, typename boost::conditional<T::has_value_semantic, const typename T::scalar_type, typename T::scalar_type>::type &, T::flags & IMAGE_LAYOUT_MASK, T::plane_count, T::row_count == 1 && T::column_count == 1>
...
{
  template<class> friend struct Image;
  template<class> friend struct image_::image_iterator;
  template<class> friend struct image_::image_traversal_pixel;
  template<class> friend struct image_::image_traversal_scalar;
  typedef T format;
  typedef Image this_type;
  typedef typename boost::conditional<T::has_value_semantic, Image<typename T::template with_scalar_type<const typename T::scalar_type>::type>, this_type>::type const_this_type;
...
public:
  ...
  typedef typename boost::remove_cv<scalar_type>::type mutable_scalar_type;
  ...
  typedef typename boost::conditional<T::dimension == 1, scalar_type, typename boost::conditional<T::row_count != 1, row_type, pixel_type>::type>::type value_type;

  /// \copydoc std::RandomAccessContainer::reference
  typedef typename boost::conditional<T::dimension == 1, reference_to_scalar, typename boost::conditional<T::row_count != 1, reference_to_row, reference_to_pixel>::type>::type reference;

  /// \copydoc std::RandomAccessContainer::pointer
  typedef typename boost::conditional<T::dimension == 1, pointer_to_scalar, typename boost::conditional<T::row_count != 1, pointer_to_row, pointer_to_pixel>::type>::type pointer;

  /// \copydoc std::RandomAccessContainer::iterator
  /// \todo what if rows ≠ 1 && columns = 1 && planes = 1 && row_step ≠ ±1 ?
  typedef typename boost::conditional<T::dimension == 1, pointer_to_scalar, image_::image_iterator<reference> >::type iterator;
  ...
  */
  ...
  row_step(void) const
  {
    return T::row_step < 0 ? -T::row_step * ((width() * depth() * sizeof(scalar_type) - 1) / std::min(1, -T::row_step) + 1) / sizeof(scalar_type) : the_row_step::get();
  }
